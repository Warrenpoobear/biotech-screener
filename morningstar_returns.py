...
Uses md.direct.returns() (the working method, not get_returns).
...
class ReturnsProvenance:
    ...
    api_method: str  # 'md.direct.returns'
    package_version: str
...
def fetch_returns(
    ...
    try:
        # Use the working method: md.direct.returns()
        # Note: This has deprecation warning but works reliably
        ...
        for idx, row in absolute_returns.iterrows():
            absolute_records.append({
                'sec_id': row['Id'],
                'date': row['Date'],
                'return_pct': str(Decimal(str(row['Return']))),  # Store as string for Decimal
            })
        ...
        if benchmark_sec_id:
            try:
                ...
                # Excess returns are optional - log but continue
...
def build_ticker_mapping(
    ...
    try:
        ...
        # This is simpler than the search method

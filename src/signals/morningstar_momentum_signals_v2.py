...
def run_acceptance_tests():
    ...
    # Check that perfect scores exceed baseline (50)
    # Note: Due to normalization bounds (Sharpe capped at +/-3, returns capped at +/-50%),
    # scores won't reach 100 even with extreme inputs - this is by design
    perfect_returns = {date: Decimal("0.01") for date in monotonic_returns.keys()}
    signals_perfect = calc.calculate_all_signals("TEST2", perfect_returns, xbi_flat, "2024-05-10")

    assert signals_perfect["composite_momentum_score"] > Decimal("60.0"), \
        f"FAIL: Strong returns should score >60, got {signals_perfect['composite_momentum_score']}"
    print(f"  PASS: Strong score = {signals_perfect['composite_momentum_score']:.2f} (>60, properly bounded)")
